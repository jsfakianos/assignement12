...
5. Choose an instance type of **t2.micro** and click **Review and Launch**.
<br/>![](Images/select_instance_type.png)
6. You'll be asked for a key pair. Select **Create a new key pair**, give it a name, and click **Download Key Pair**.
...
9. Restrict permissions on the key. ssh will require this later:
...
Even on the free tier, AWS can bill you for usage over the free rate limits. I recommend you go to [https://console.aws.amazon.com/billing/](https://console.aws.amazon.com/billing/) and configure **billing alerts** that will let you know if you start getting charges on your account. If you make a mistake or start getting unexpected traffic you'll be able to react quickly and avoid getting any surprising charges.

## Set up an Elastic IP address
By default, your EC2 instance will be assigned a non-static IP address. When the instance is restarted or stopped, it will lose it's public IP. To deal with this we will configure an Elastic IP so that our lamp will always have an endpoint to talk to.
